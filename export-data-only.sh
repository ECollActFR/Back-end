...
# Script d'export des données uniquement (INSERT sans DROP TABLE ni CREATE TABLE)
# Génère un fichier SQL avec uniquement les INSERT
...
OUTPUT_FILE="data_only_${DB_NAME}_${TIMESTAMP}.sql"
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Export des données uniquement (INSERT)${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
if [ -f "${OUTPUT_FILE}" ]; then
    ...
    echo -e "${BLUE}Nombre de lignes :${NC} ${LINES_COUNT}"
    echo -e "${BLUE}Nombre d'INSERT :${NC} ${INSERT_COUNT}"
    echo ""

    # Afficher un aperçu du contenu
    echo -e "${BLUE}Aperçu du fichier (15 premières lignes) :${NC}"
